...
function getSpeedLeft(a){
    ...
    v0_L  = v0 * a / getLeftAngle(-leftPhi0); //speed prop. with angle
    vx0_L = v0_L * sin(a);
    vy0_L = v0_L * cos(a);
    xMax_L = v0_L * v0_L / g ;	
...
function getSpeedRight(a){
    ...
    v0_R  = v0 * a / getRightAngle(-rightPhi0); //speed prop. with angle
    vx0_R = v0_R * sin(a);
    vy0_R = v0_R * cos(a);
    xMax_R = v0_R * v0_R / g ;
...
function ballRightRoll_OnFloor(){
    yball_R = r_ball;
...
function ballLeftFly() {
    ...
    if (yball_L >= r_ball) {
        ...
        t_left = 0;         // reset time
        x0L = xball_L;      
        vx0_L = vx0_L;      // Speed transfer, no friction remains unchanged
        status_left = 2;    // ball landed, roll on the floor
